...
### v00.01.05
1. [I] Implement OS URL handler registration with `+register`. [Feature-07] Implemented in v00.01.05
  - [I] Once registered, a URL with the format `goto://<alias>` will invoke `mwt-g +b` to launch the underlying URL in the browser. Implemented in v00.01.05
  - [I] The registration path should be the path of the script, wherever it is. Implemented in v00.01.05
  - [I] Implement the tests using Pester. Implemented in v00.01.05
...
1. [R] update Register-UrlSchemeHandler() in mwt-g.ps1 with the AppleScript template file "mwt-g-url-handler.applescript.template"
1. Implement tests using Pester
    - [I] Verify with the OS that `goto://` is indeed registered. Implemented in v00.01.05
    - [I] Verify that on a Mac, `open goto://y` does not return an error. Implemented in v00.01.05
    - [I] Fix T0018 - it should not be returning any text at all. Implemented in v00.01.05
