...
1. Only execute tasks marked with `[R]`.
  ...
  - `[R]` indicates the AI has implemented the code; set this once the work is completed in this session.
  - `[R]` indicates the user has signed off on the implementation.
1. Feature tracking is done using the format `Feature-00[ ]`
  - `[ ]` means that the feature has not been implemented.
  - `[R]` means that the feature is implemented. The AI shall annotate the feature description with the version that it was implemented in.
  - `[R]` is the human sign-off
...
- Feature-07[ ]: Register URL scheme
  ...
  g +register
  ```
  - Registers the custom `goto://` URL scheme with the OS so links like `goto:/y` open via this tool
...
- Only mark a task as `[R]` after the tests have been executed and passed.
...
1. [R] Implement the ability to add a basic URL (`g g https://www.google.com`). [Feature-01] Implemented in v00.01.01
   - [R] add `+overwrite-alias` with `always`,`never`,`ask` options. Implemented in v00.01.01
   - [R] add `+quiet` to supress any messages. Implemented in v00.01.01
   - [R] app should display what it's doing (and why, if there are flags involved.) Implemented in v00.01.01
1. [R] Implement `+n` (display URL) and make it the default. [Feature-06] Implemented in v00.01.01
1. [R] Implement alias listing with `+list`. [Feature-05] Implemented in v00.01.01
1. [R] Rework the files:
       - [R] All configurations should be in configuration.toml (defaults respected by tool)
       - [R] All `+` options should be stored with defaults
           - [R] +overwrite-alias always
       - [R] Move the aliases to aliases.toml
        - [R] Create the configuration.toml file if it doesn't exist. Implemented in v00.01.01
1. [R] Implement tests using Pester
      - [R] `mwt-g g https://www.google.com` should add the alias to the settings file. [T0001]
      - [R] `mwt-g g` should display https://www.google.com [T0002]
      - [R] `mwt-g gg` throws an error [T0003]
      - [R] `mwt-g +list` should list all the aliases added [T0004]
      - [R] `mwt-g g https://news.google.com/` should overwrite the alias. Implemented in v00.01.01 [T0010]
       - [R] `mwt-g +quiet` should output nothing. [T0007]
       - [R] `mwt-g +overwrite-alias never g https://www.google.com` should refuse to overwrite the alias `g`. [T0006]
       - [R] `mwt-g +overwrite-alias always g https://www.gmail.com` should overwrite the alias `g`. Implemented in v00.01.01 [T0011]
       - [R] Test that the configuration file exists. Implemented in v00.01.01 [T0008]
      - [R] `mwt-g gx https://xxx.google.com/` should throw errors when trying to visit with 'mwt-g gx'. Implemented in v00.01.01 [T0012]

### v00.01.02
1. [R] Implement `+c` (fetch via curl). [Feature-06] Implemented in v00.01.02
1. [R] Implement tests using Pester
  - [R] Provide the tests a unique code T0000
    - `+c` fetch test: [T0009]


### v00.01.03
1. [R] Implement `+b` (open in browser). [Feature-06] 
1. [R] Implement OS URL handler registration with `+register`. [Feature-07] 
  - [R] Once registered, a URL with the format `goto://<alias>` will invoke `mwt-g +b` to launch the underlying URL in the browser. 
  - [R] The registration path should be the path of the script, wherever it is. 
  - [R] Implement the tests using Pester.
1. [R] Implement bin/tests-list.ps1 that will list each test and what it does. 
1. [R] Implement bin/tests-run-test.ps1 that will run a specific by TESTNUMBER T0000.
1. Implement tests using Pester
  - [R] Verify with the OS that `goto://` is indeed registered.
  - [R] Verify that on a Mac, `open goto://y` does not return an error.
  - [R] Fix T0018 - it should not be returning any text at all. 
